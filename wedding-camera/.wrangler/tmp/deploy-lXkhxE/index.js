...
var ADMIN_USERNAME = "a-n";
var ADMIN_PASSWORD = "4-4-26";
var UNLOCK_SCHEDULE = {
  year: 2026,
  month: 3,
  day: 22,
...
__name(getUnlockDate, "getUnlockDate");
...
__name(formatUnlockDate, "formatUnlockDate");
...
__name(getUnlockState, "getUnlockState");
var index_default = {
      ...
      try {
        ...
        await env.DB.prepare(
          `
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
          `
        ).bind(sessionId, event.id, deviceToken, null, 0, event.max_shots, "active", now, now).run();
      ...
      try {
        ...
        await env.DB.prepare(
          `
            ...
            VALUES (?, ?, ?, ?, ?)
          `
        ).bind(photoId, session.event_id, session.id, r2Key, now).run();
        ...
        await env.DB.prepare(
          `
            ...
            WHERE id = ?
          `
        ).bind(newShotsTaken, newShotsRemaining, newStatus, now, session.id).run();
